 ...
 *
 * Until now each of the nine pages carried its own `tailwind.config` inline.
 * Seven different configs meant seven different meanings for the same class
 * name: `text-cyan-400` was #22d3ee on Data Literacy (which overrode the
 * built-in cyan ramp) and Tailwind's stock #22d3ee-adjacent value everywhere
 * else, `bg-brand-500` was a #3b82f6 blue on Day-to-Day and undefined on Data
 ...
 * handed this same object to the Tailwind Play CDN, which then compiled the
 * utilities on every visitor's device on every page view. The theme did not
 * change when it moved here -- only who reads it and when.
 *
...
/** The `theme.extend` block of the simulators' Tailwind config. */
export const SIMULATOR_THEME_EXTEND = {
  /* ---------------------------------------------------------------
   * One typeface.
   *
  ...
  gridTemplateColumns: {
    /* Data Literacy's scenario progress bar is fifteen segments wide, one per
       scenario, and its markup has always said `grid-cols-5 sm:grid-cols-15`.
       Tailwind's stock scale stops at 12, so `sm:grid-cols-15` was never a
       real class and the bar has been rendering as three rows of five at every
       width. The Play CDN gave no sign of it -- an unmatched utility compiles
       to nothing and looks exactly like a utility that did not need to apply.
       Declaring it here is what the markup was asking for. */
  ...
  /* No `animation` or `keyframes` entries.
     Data Literacy used to ask for `animate-pulse-glow` and `animate-float`,
     ...
     since the compiler only emits utilities it finds in the markup. */
};
