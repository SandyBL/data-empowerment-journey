...
deck: Four scenario simulators, a public board for each, and the patterns that keep coming back when people have to decide under time pressure — plus an honest account of what a public board cannot tell you.
description: Public results from four data governance simulators, with the key lessons: how governance instincts fail under pressure, why ownership defaults to IT, and what a leaderboard cannot measure.
...
**Data Literacy** is fifteen points across governance, analytics, AI and automation, bias awareness and data culture. It is the only one of the four that asks you to decide about culture rather than to define it, and the only one that hands you a second number: the value it estimates you unlocked from your data assets.

**CDMP Exam Practice** is the odd one out, and deliberately so: ten questions drawn at random from a hundred covering the eleven knowledge areas of the DAMA-DMBOK wheel and the six the Fundamentals exam adds around it, scored out of 1000 in the same format the real exam uses. It is knowledge rather than judgment, which is why it is the only board here where a fast, confident run is a good sign. The questions and options are in English on all three language versions of the page, because the official exam is only in English and practising the vocabulary in translation would practise the wrong vocabulary.

None of the four sees all five [data maturity](/en/glossary/data-maturity/) pillars. Day-to-Day and Ownership are both blind to [data culture](/en/glossary/data-culture/); Literacy is blind to metadata; CDMP Exam Practice covers metadata and quality directly but is read as blind to culture, because knowing which chapter defines data culture is not evidence that your organization has one. That is what a ten-question exercise is, not a flaw waiting to be fixed, and it sets the rule the boards are read under: a dimension nobody measured is reported as unmeasured, never as weak. Inferring a culture problem from a RACI exercise would be inventing a finding.
...
**[See how a workshop runs →](/en/workshops/)**: the format, the debrief, which of the four scenarios fits which room, what the facilitator report contains, and how sessions are priced. Start here if you are still deciding whether this fits.
