...
Esta página es lo que sale por el otro extremo. No el tablero en sí —eso está en cada simulador— sino la distribución que hay detrás, y las lecciones que sobreviven a mirarlas con cuidado.
...
Por eso las cifras de arriba reportan la distancia entre la mejor partida y la mediana, y por eso el informe del facilitador de una sesión privada abre con el desacuerdo y no con el ganador. En un taller los desacuerdos *son* la sesión: el tablero va a la pantalla, y entonces las dos personas que eligieron cosas opuestas se explican la una a la otra usando su propia organización como ejemplo.
...
Todo lo anterior es o una propiedad de cómo están construidos los simuladores o un patrón de haberlos pasado con grupos reales. Lo que *no* es es un diagnóstico de ninguna organización concreta — y merece la pena ser preciso sobre el motivo, porque es una decisión de gobernanza de datos y no una limitación del producto.
...
Si el taller es una pieza de algo más grande, [los servicios de consultoría](/es/consulting/) explican cómo encaja normalmente — casi siempre como movimiento de apertura del diseño de un modelo operativo, porque una sala que acaba de discutir sobre propiedad del dato se engancha a una matriz de derechos de decisión como no lo hace nunca una sala que no ha discutido.
